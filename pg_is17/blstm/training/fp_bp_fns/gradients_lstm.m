...
function [gWz,gRz,gbz,gWi,gRi,gpi,gbi,gWf,gRf,gpf,gbf,gWo,gRo,gpo,gbo] = gradients_lstm(X,hm,cm,dom,dfm,dim,dzm,nl,dir_flag)

if strcmp(dir_flag,'frnn')
   hpm = [zeros(1,nl(2));hm(1:end-1,:)];
   cpm = [zeros(1,nl(2));cm(1:end-1,:)];
else
   hpm = [hm(2:end,:);zeros(1,nl(2))];
   cpm = [cm(2:end,:);zeros(1,nl(2))];
end
