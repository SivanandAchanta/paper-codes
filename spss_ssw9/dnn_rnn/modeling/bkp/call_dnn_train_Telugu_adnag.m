...
nl = [din 500 500 500 500 500 500 500 500 500 dout];
nh = length(nl) - 1; % number of hidden layers
f = [ 'R' 'R' 'R' 'R' 'R' 'R' 'R' 'R' 'R' 'L'];
...
l1 = 0.00;
l2 = 0.0000;
rhoada_vec = [0.95 0.98 0.9];
epsada_vec = [1e-5 1e-6 1e-4];
eta = 1e-3;
cfn = 'ls';
a_tanh = 1.7159;
b_tanh = 2/3;
...
for mfi = 1:length(mf_vec)
    ...
    arch_name = strcat(arch_name1,'_bs',num2str(train_batchsize),'_rho_eps_mf',num2str(rhoada),num2str(epsada),num2str(mf));
