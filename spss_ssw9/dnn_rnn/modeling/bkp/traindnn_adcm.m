...
addpath('../../../MNIST/mfiles');

% open error text file
fid = fopen(strcat('../err/err_',arch_name,'.err'),'w');
...
for NE = 1:numepochs
    ...
    for i = 1:val_numbats
        ...
        clear ol;
        [me] = ce(ol_mat,val_batchtargets(:,:,i));
        clear ol_mat;
    ...
    if valerr(NE) < best_val_loss
        ...
        for i = 1:test_numbats
            ...
            clear ol;
            me = ce(ol_mat,test_batchtargets(:,:,i));
            clear ol_mat;
    ...
    for i = 1:train_numbats
        ...
        switch cfn
            case 'ls'
                costder = -(train_batchtargets(:,:,i) - ol_m);
        ...
        % adadelta equations (step 4)
        Gpmsgbt = rhoada*(Gpmsgbt) + (1-rhoada)*(gb.^2);
        GpmsgWt = rhoada*(GpmsgWt) + (1-rhoada)*(gW.^2);
        
        
        % step 5
        rmsgbt = sqrt(Gpmsgbt + epsada);
        rmsgWt = sqrt(GpmsgWt + epsada);
        rmsxbt = sqrt(Gpmsxbt + epsada);
        rmsxWt = sqrt(GpmsxWt + epsada);
        
        Gdb = -(rmsxbt./rmsgbt).*gb;
        GdW = -(rmsxWt./rmsgWt).*gW;
        
        % step 6
        Gpmsxbt = rhoada*(Gpmsxbt) + (1-rhoada)*(Gdb.^2);
        GpmsxWt = rhoada*(GpmsxWt) + (1-rhoada)*(GdW.^2);
