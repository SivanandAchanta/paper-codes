...
# MATLAB and STRAIGHT
MATLAB=$2' -nodisplay -nosplash -nojvm'
STRAIGHT=$3
...
SAMPFREQ=$5   # Sampling frequency (48kHz)
FRAMESHIFT=$6 # Frame shift in point (240 = 48000 * 0.005)
FREQWARP=$7   # frequency warping factor
GAMMA=$8      # pole/zero weight for mel-generalized cepstral (MGC) analysis
MGCORDER=$9   # order of MGC analysis
LNGAIN=${10}     # use logarithmic gain rather than linear gain
LOWERF0=${11}    # lower limit for f0 extraction (Hz)
UPPERF0=${12}    # upper limit for f0 extraction (Hz)
...
ls wav_16k > wavlist.txt
WAVFILELIST="wavlist.txt"
cat $WAVFILELIST |
awk '{print $1}' |
while read i
do   
   ...
   echo $fname wav2raw
   ch_wave -c 0 -F 32000 -otype raw wav_16k/$i | x2x +sf | interpolate -p 2 -d | ds -s 43 | x2x +fs > raw/$fname.raw
done

# Extracting f0, aperiodicity, and spectra using STRAIGHT
mkdir -p wav sp ap f0
frameshift=`expr 1000 \* $FRAMESHIFT / $SAMPFREQ`; \
echo "path(path,'$STRAIGHT');"                      >  extract.m; \
echo "prm.F0frameUpdateInterval=$frameshift;"      >> extract.m; \
echo "prm.F0searchUpperBound=$UPPERF0;"             >> extract.m; \
echo "prm.F0searchLowerBound=$LOWERF0;"             >> extract.m; \
echo "prm.spectralUpdateInterval=$frameshift;"     >> extract.m; \
echo ""                                               >> extract.m
for raw in raw/*.raw; do \
	...
	if [ -s $raw -a $min -gt -32768 -a $max -lt 32767 ]; then \
		...
		echo "[x,fs]=wavread('$wav');"             >> extract.m; \
		echo "[f0, ap] = exstraightsource(x,fs,prm);" >> extract.m; \
        echo "[sp] = exstraightspec(x, f0, fs, prm);" >> extract.m; \
		echo "ap = ap';"                              >> extract.m; \
		echo "sp = sp';"                              >> extract.m; \
		echo "save '$f0' f0 -ascii;"               >> extract.m; \
		echo "save '$ap' ap -ascii;"               >> extract.m; \
		echo "save '$sp' sp -ascii;"               >> extract.m; \
		echo ""                                       >> extract.m; \
	fi; \
done; \
echo "quit;"                                          >> extract.m
