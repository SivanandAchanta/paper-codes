...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% load weights
arch_name = '347L1000R1000R150L_l25e-05_lr0.03_mf0.9_yi_gv1_27';
...
for li = 1:20%length(testfiles)-1
    ...
    %     % GV using DNN-WA
    %     % fp ddn-wa for gv
    %     [gv_hcm,gv_ah,gv_alpha,gv_c,gv_sm,gv_ym] = fp_cpu(X(:,invec_gv),Wi,bi,Wa,ba,Wh,bh,Wo,bo,f_gv);
    %     if outtmvnf_gv
    %         I1 = bsxfun(@times,gv_ym,gvmaxv(outvec_gv));
    %         I1 = bsxfun(@plus,I1,gvminv(outvec_gv));
    %         gv_ym = I1;
    %     end
    %
    ...
    %     end
    %     dix_vec = diff(ix_vec);
    %     dix_vec_int = find(dix_vec);
    
    %     figure;surf(double(repmat(gv_alpha,10,1)),'edgecolor','none');view(0,90); colormap(1-gray); axis tight; ...
    %         set(gca,'XTick',[1:length(gv_alpha)]);set(gca,'XTickLabel',(phns_file));
    
    %     figure(1);surf(double(repmat(gv_alpha,3,1)),'edgecolor','none');view(0,90); colormap(1-gray); axis tight; ...
    %         set(gca,'XTick',dix_vec_int);set(gca,'XTickLabel',(phns_file(dix_vec_int))); set(gca,'YTick',[]);
    %     pause
    %     imshow(repmat(gv_alpha,10,1),[]);
    %     xlabel(phns_file')
    ...
    % AbS
    if syn_flag; synth_st(f01',M1,ap1,fs,prm,abssynthdir,str,'_mgcfullabs.wav'); end;
    ...
    if syn_flag; synth_st(f01',M1,ap1,fs,prm,spssynthdir,str,'_speconly_nomlpg.wav'); end;
    ...
    if syn_flag; synth_st(f01',M1_mlpg,ap1,fs,prm,spssynthdir,str,'_speconly_chol.wav'); end;
