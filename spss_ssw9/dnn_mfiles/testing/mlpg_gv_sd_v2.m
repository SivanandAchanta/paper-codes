function [Y,Ygv,Ygv_2] = mlpg_gv_sd_v2(S,m,mu_v,p_v,mu_v_2)
...
for i = 1:d
    ...
    cp_2 = sqrt((mu_v_2(i))/vc)*(cp - mean(cp));
    ...
    for j = 1:num_iters
        ...
        % type2 gv (using predicted mean from the dnn-wa network)
        t11 = -Rq*cp_2;
        %         t1 = omega*(t11/2 + rq);
        t2 = omega*(t11 + rq);
        ...
        % compute var
        vc = var(cp_2);
        v = c1*(vc - mu_v_2(i))*(cp_2 - mean(cp_2));
